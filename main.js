...
const hentaiWebsites = [{
    type: 'list',
    name: 'mode',
    message: 'What website would you like to work on ?',
    choices: ['HentaiNexus', 'HentaiCafe', 'Nhentai']
}];
let hentaiNexusOperations = [{
    type: 'list',
    name: 'option',
    message: 'What would you like to do?',
    choices: ['Get the website data as an SQLite database', 'Get hentai thumbnail from the database', 'Download some hentai']
}];
let databaseOperatioons = [{
    type: 'input',
    name: 'pageStartingNumber',
    message: 'What page would you like to start from ?',
    default: '1'
}];
let downloadStartingPoint = [{
    type: 'input',
    name: 'downloadStartingNumber',
    message: 'What page would you like to start from ?',
    default: '1'
}];
let downloadOptions = [{
    type: 'list',
    name: 'downloadMode',
    message: "Which downlaading methode you'd like to use ?",
    choices: ['By title', 'By Hentai Id', 'By tag', 'By artist', 'By magazin']
}];
...
let hentaiCafeURLdownload = [{
    type: 'input',
    name: 'HCdownloadURL',
    message: 'Insert your desired hentai url here :',
    default: 'https://hentai.cafe/hazuki-yuuto-locker-trap/'
}];
// 
let nhentaiDownloadMod = [{
    type: 'list',
    name: 'nhentaiMode',
    message: 'Choose your download mode :',
    choices: ['Url', 'Code']
}];
            ...
            inquirer.prompt(hentaiNexusOperations).then(answers2 => {
                ...
                switch (res2.option) {
                    ...
                    case hentaiNexusOperations[0].choices[2]:
                        ...
                        break;
                    case hentaiNexusOperations[0].choices[1]:saveThumb();break;
                }
            });
            break;
        case "HentaiCafe":
...
async function getData(currentPage) {
    db.serialize(function () {
        db.run("CREATE TABLE IF NOT EXISTS Hentai (id INTEGER PRIMARY KEY,Title Text,Artist Text,Language Text,Magazine Text,Circle Text,Event Text,Parody Text,Publisher Text,Pages Integer,Tags Text,Description Text,FirstPage Text,Cover Text)");
    });
    //
...
async function nhentaiDownloader(data) {
    ...
    data.Name = data.Name.replace(/:/g, ' ').replace('?', ' ').replace('?', ' ').replace('|',' ').replace('"',' ').replace('"',' ');
